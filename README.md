...
Embedded/app runtimes where `process.execPath` is not Bun can override the probe subprocess explicitly:
...
The embedded inline probe path ignores unrelated per-device USB access/open
errors while searching for the requested Fastboot interface, matching the
library's normal discovery behavior.
